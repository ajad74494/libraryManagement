	...
	public LibrarianLogin() {
		...
		btnLogin.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String queryString = "SELECT name, password FROM librarian";
				...
				try {
					while (results.next()) {
						String staffname = results.getString(name);
						String password1 = results.getString(password);

						if ((name.equals(staffname)) && (password.equals(password1))) {
		...
		gl_contentPane.setVerticalGroup(gl_contentPane.createParallelGroup(Alignment.LEADING).addGroup(gl_contentPane
				.createSequentialGroup().addComponent(lblAdminLoginForm).addGap(26)
